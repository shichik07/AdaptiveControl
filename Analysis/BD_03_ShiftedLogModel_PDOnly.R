...
prior_weakly_informed_LW <- c(
  ...
  prior(normal(0,0.5), class = b, coef = Analysis_typeMC), # Priors of Contrasts for CO
  prior(normal(0,0.5), class = b, coef = Congruencycongruent),
  prior(normal(0,0.5), class = b, coef = Congruencycongruent:Analysis_typeMC),
...
m2_PD_LW <- bf(RT ~ 1 + Congruency + Analysis_type + Congruency*Analysis_type  + (Subject|Congruency*Analysis_type) + (1|Subject) + (1|Item)) 
