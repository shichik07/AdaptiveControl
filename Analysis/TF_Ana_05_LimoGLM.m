...
data_C = squeeze(Betas(ch_t,:,:,4));

max_scale = 5;%max(LW_MI.pl.congruent,[], 'all')
min_scale = 5;%min(LW_MI.pl.congruent,[], 'all')
freq_low = min(TF_non_phase.Frequencies);
freq_up  = max(TF_non_phase.Frequencies);

figure(1)
contourf(TF_non_phase.Time,TF_non_phase.Frequencies,data_C,40,'linecolor','none')
...
% baseline correction for the non-phase corrected data too!!
