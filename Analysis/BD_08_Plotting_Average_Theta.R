...
Plt_sl_ISPCE <- ggplot(data = SL_ISPC, aes(x= Time , y = Pw, color = Prop_type, linetype = Congruency)) +
  ...
  geom_line(aes(x = Time, y = sig-2.5),na.rm = TRUE,size = 2, color = "black", show.legend =FALSE) +
  guides(fill = "none") +
...
save_n <- "Average_theta_ISPC_SL.png"
...
save_n <- "Average_theta_ISPC_RL.png"
ggsave(path = save_path, filename = save_n,  dpi=300)
...
Plt_sl_LWPCE <- ggplot(data = SL_LWPC, aes(x= Time , y = Pw, color = Prop_type, linetype = Congruency)) +
  ...
  guides(fill = "none")  +
  geom_line(aes(x = Time, y = sig-3),na.rm = TRUE,size = 2, color = "black", show.legend =FALSE) +
  guides(color = guide_legend(override.aes = list(
    size = 6,
    linetype = c(0, 0),
    fill = c("#E16A86","#00A6CA"),
...
save_n <- "Average_theta_LWPC_SL.png"
ggsave(path = save_path, filename = save_n,  dpi=300)
...
save_n <- "Average_theta_LWPC_RL.png"
ggsave(path = save_path, filename = save_n,  dpi=300)


# Now let us combine the plots for the ISPC data
LWPCE_comb <- ggarrange(Plt_sl_LWPCE + rremove("legend"), Plt_rl_LWPCE + rremove("legend"),
...
ISPCE_comb <- ggarrange(Plt_sl_ISPCE + rremove("legend"), Plt_rl_ISPCE + rremove("legend"),
                        ncol = 2, nrow = 1, align = "h", widths = c(1,1))
