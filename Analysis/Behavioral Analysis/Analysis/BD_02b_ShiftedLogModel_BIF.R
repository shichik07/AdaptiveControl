...
modify_formula <- function(model_t){
  # check which effect we are working with
  if (isTRUE(grep("LW",model_t)==1)){
    forms =  LW_formulas
    c_names = LW_contrast_names
  } else if (isTRUE(grep("IS",model_t)==1)){
    forms =  IS_formulas
    c_names = IS_contrast_names
  }
  # define models based on group and model type for BIF calculation 
  if (isTRUE(grep("PD",model_t)==1)){
    if (isTRUE(grep("Intercept",model_t)==1)){
      temp_cont <- forms[-4:-6]
      temp_varn <- c_names[-4:-6]
    } else if (isTRUE(grep("Congruency",model_t)==1)){
      temp_cont <- forms[c(-5,-6)]
      temp_varn <- c_names[c(-5,-6)]
    } else if (isTRUE(grep("BlocknItem",model_t)==1)){
      temp_cont <- forms[c(-4,-6)]
      temp_varn <- c_names[c(-4,-6)]
    } else if (isTRUE(grep("2Factor",model_t)==1)){
      temp_cont <- forms[c(-6)]
      temp_varn <- c_names[c(-6)]
    }
    
  } else if (isTRUE(grep("CO",model_t)==1)){
    if (isTRUE(grep("Intercept",model_t)==1)){
      temp_cont <- forms[-1:-3]
      temp_varn <- c_names[-1:-3]
    } else if (isTRUE(grep("Congruency",model_t)==1)){
      temp_cont <- forms[c(-2,-3)]
      temp_varn <- c_names[c(-2,-3)]
    } else if (isTRUE(grep("BlocknItem",model_t)==1)){
      temp_cont <- forms[c(-1,-3)]
      temp_varn <- c_names[c(-1,-3)]
    } else if (isTRUE(grep("2Factor",model_t)==1)){
      temp_cont <- forms[c(-3)]
      temp_varn <- c_names[c(-3)]
    }
  }
  #return contrast matrix and variable names
  return(list(varnames = temp_varn, contrast_mat = temp_cont))
}
...
for(mods in LW_mods){
  # get contrast mat and variable names
  mat_obj <- modify_formula(mods)
  # first create the contrast matrix
  temp_mat <- hypr(mat_obj$contrast_mat,
                   levels = LW_levels)
  # next add the appropriate variable names
  names(temp_mat) <- mat_obj$varnames
  
  # Lastly rename the contrast matrix
  assign(mods, temp_mat)
}
...
# define a list which priors to exclude, based on model
prior_exc <- list(
  c(-5,-6,-7), # CO intercept priors
  c(-6,-7), # CO congruency priors
  c(-5,-7), # CO Listwide priors
  c(-7), # CO two factor model priors
  c(-8,-9,-10),
  c(-9,-10),
  c(-8,-10),
  c(-10)
...
for(mods in 1:length(LW_mods)){
  ...
  # define prior
  LW_weakly <- prior_weakly_informed_LW[prior_exc[[mods]],]
...
for(mods in 1:length(IS_mods)){
  ...
  # define prior
  IS_weakly <- prior_weakly_informed_IS[prior_exc[[mods]],]
  # get save name for variable
...
prior_exc_dia <- list(
  c(-2,-3,-4), # CO intercept priors
  c(-3,-4), # CO congruency priors
  c(-2,-4), # CO Listwide priors
  c(-4), # CO two factor model priors
  c(-5,-6,-7),
  c(-6,-7),
  c(-5,-7),
  c(-7)
...
for(mods in 1:length(LW_mods)){
  ...
  # define prior
  LW_informed <- prior_informed_LW[prior_exc_dia[[mods]],]
  # get save name for variable
...
for(mods in 1:length(IS_mods)){
  ...
  # define prior
  IS_informed <- prior_informed_IS[prior_exc_dia[[mods]],]
  # get save name for variable
