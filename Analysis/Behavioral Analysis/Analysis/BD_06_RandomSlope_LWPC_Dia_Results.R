  ...
  if (group == "PD"){
    for (part in part_nr) {
      ...
      LW_MC_C <- exp((m_post$b_Intercept + eval(parse(text = var_intercept))) +
                       ...
                       0.5* (m_post$b_Contrast_LWPD_LW_Block + eval(parse(text = var_Interaction))) + 
                       m_post$sigma/2)
    ...
    for (part in part_nr) {
      ...
      LW_MC_C <- exp((m_post$b_Intercept + eval(parse(text = var_intercept))) +
                       ...
                       0.5* (m_post$b_Contrast_LWCO_LW_Block + eval(parse(text = var_Interaction))) + 
                       m_post$sigma/2)
      LW_MC_I <- exp((m_post$b_Intercept + eval(parse(text = var_intercept))) - 
                       0.5* (m_post$b_Contrast_LWCO_Congruency + eval(parse(text = var_Congruency))) + 
                       0.5 * (m_post$b_Contrast_LWCO_Listwide + eval(parse(text = var_Listwide)))  - 
...
PD_effects <- conditional_effect_calc_shift(model = fit_ShiftedLog_PDOnly_diagnostic_LW, 
                              group = "PD", 
                              part_nr = part_nr_PD)
write.csv(PD_effects,"Participant_specific_effect_summary_PD.csv", row.names = FALSE)

CO_effects <- conditional_effect_calc_shift(model = fit_ShiftedLog_COOnly_diagnostic_LW, 
                                           group = "CO", 
                                           part_nr = part_nr_CO)

write.csv(CO_effects,"Participant_specific_effect_summary_CO.csv", row.names = FALSE)

# Summarize both tables

Full_data <- PD_effects %>%
  bind_rows(CO_effects)

write.csv(Full_data,"Participant_specific_effect_summary.csv", row.names = FALSE)
