...
t = -200/1000: 1/(EEG.srate/2) : 1000/1000; % divide sampling rate because we downsample data
pre_trial_bs_idx = dsearchn(t', -200/1000): dsearchn(t', -100/1000); % divide sampling rate because we downsample data
full_trial_bs_idx = dsearchn(t', 0/1000): dsearchn(t', 1000/1000); 
BS = "post"; %"post" select whether your want the pre or post baseline correction

if BS == "pre"
    bsl_idx = pre_trial_bs_idx;
...
for sub = 2:Part_N
    ...
    for p_type = 1%:2 % do this for the phase locked and non-phase locked data
        ...
        %base_power = squeeze(mean(data(:,:,:,:),4));
