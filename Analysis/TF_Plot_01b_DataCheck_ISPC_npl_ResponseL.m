...
examp                       = fullfile(dirs.home, Participant_IDs{4}, strcat(Participant_IDs{4} ,"_LWPC_MC_I_frequency_data_pl_RL_bslC_RL.mat"));
data                        = load(examp);
...
contourf(Time,freqs,data_C,40,'linecolor','none')
set(gca,'clim',[min_scale, max_scale],'yscale','log','ytick',logspace(log10(freq_low),log10(freq_up),6),'yticklabel',round(logspace(log10(freq_low),log10(freq_up),6)*10)/10)
title('Power - Parkinson Congruence MC (FCz) LWPC')
colormap(jet)
colorbar
...
sgtitle('Theta-band Congruency Effect MI Control Group ISPC')
...
h = colorbar;
set(h,'Position',[0.93 0.05 0.03 0.9])
sgtitle('Theta-band Congruency Effect MC Control Group ISPC')
...
h = colorbar;
set(h,'Position',[0.93 0.05 0.03 0.9])
sgtitle('Theta-band Interaction Control Group ISPC')
...
h = colorbar;
set(h,'Position',[0.93 0.05 0.03 0.9])
sgtitle('Theta-band Congruency Effect MC PD Group ISPC')
...
h = colorbar;
set(h,'Position',[0.93 0.05 0.03 0.9])
sgtitle('Theta-band Congruency Effect MI PD Group ISPC')
...
h = colorbar;
set(h,'Position',[0.93 0.05 0.03 0.9])
sgtitle('Theta-band Interaction PD Group ISPC')
